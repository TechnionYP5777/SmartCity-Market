...
private IRegisteredCustomer customer;
	...
	@Test
	public void OfferSpecailSaleForProductSuccessfulTest() {	
		
		try {
			Mockito.when(
				clientRequestHandler.sendRequestWithRespond(new CommandWrapper(CustomerDefs.loginCommandSenderId, CommandDescriptor.OFFER_SPECIAL_SALE_FOR_PRODUCT,
						Serialization.serialize(sale)).serialize()))
		...
		try {
			testSale = customer.offerSpecailSaleForProduct(sale);
	...
	@Test
	public void OfferSpecailSaleForProductCriticalErrorTest() {
		try {
			Mockito.when(clientRequestHandler.sendRequestWithRespond(
					new CommandWrapper(CustomerDefs.loginCommandSenderId, CommandDescriptor.OFFER_SPECIAL_SALE_FOR_PRODUCT,
							Serialization.serialize(sale)).serialize()))
		...
		try {
			customer.offerSpecailSaleForProduct(sale);
	...
	@Test
	public void OfferSpecailSaleForProductCustomerNotConnectedTest() {
		try {
			Mockito.when(clientRequestHandler.sendRequestWithRespond(
					new CommandWrapper(CustomerDefs.loginCommandSenderId, CommandDescriptor.OFFER_SPECIAL_SALE_FOR_PRODUCT,
							Serialization.serialize(sale)).serialize()))
	...
	@Test
	public void OfferSpecailSaleForProductCatalogDoesNotExistTest() {
		try {
			Mockito.when(clientRequestHandler.sendRequestWithRespond(
					new CommandWrapper(CustomerDefs.loginCommandSenderId, CommandDescriptor.OFFER_SPECIAL_SALE_FOR_PRODUCT,
							Serialization.serialize(sale)).serialize()))
	...
	@Test
	public void OfferSpecailSaleForProductInvalidParameterTest() {
		try {
			Mockito.when(clientRequestHandler.sendRequestWithRespond(
					new CommandWrapper(CustomerDefs.loginCommandSenderId, CommandDescriptor.OFFER_SPECIAL_SALE_FOR_PRODUCT,
							Serialization.serialize(sale)).serialize()))
