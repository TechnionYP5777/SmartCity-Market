...
public class GetProductPackageAmountTest {
	...
	@Test void GetProductPackageAmountSuccesfulTest() {
		int retAmmount = 0; 
		try {
	...
	@Test void GetProductPackageAmountInvalidParameterTest() {
		try {
			Mockito.when(clientRequestHandler.sendRequestWithRespond(
					(new CommandWrapper(WorkerDefs.loginCommandSenderId, 
							CommandDescriptor.GET_PRODUCT_PACKAGE_AMOUNT,
	...
	@Test void GetProductPackageAmountEmployeeNotConnectedTest() {
		try {
			Mockito.when(clientRequestHandler.sendRequestWithRespond(
					(new CommandWrapper(WorkerDefs.loginCommandSenderId, 
							CommandDescriptor.GET_PRODUCT_PACKAGE_AMOUNT,
	...
	@Test void GetProductPackageAmountProductPackageDoesNotExistTest() {
		try {
			Mockito.when(clientRequestHandler.sendRequestWithRespond(
					(new CommandWrapper(WorkerDefs.loginCommandSenderId, 
							CommandDescriptor.GET_PRODUCT_PACKAGE_AMOUNT,
