...
public class SmartcodeParser {
  ...
  private static String SERIAL_NUMBER_IDENTIFIER = "21";
  ...
  private  static SmartCode codeToSmartcode(String scannedCode){
    ...
    while (scannedCode.length() == 0){
      ...
      if (scannedCode.startsWith(EXPIRATION_DATE_IDENTIFIER)){
        ...
        //check if the expiration date already inserted or the field is too small or not all digits
        if (flgDateFilled || scannedCode.length() < 6 || scannedCode.matches("\\d{" + EXPIRATION_DATE_LEN+"}"+".*"))
          return null;
        ...
        continue;
      } else if (scannedCode.startsWith(SERIAL_NUMBER_IDENTIFIER)){
        //eats the field code
        scannedCode = scannedCode.substring(SERIAL_NUMBER_IDENTIFIER.length());
        //check if the expiration date already inserted or the field is too small or not all digits
        if (flgSerialFilled)
          return null;
        ...
        $.setBarcode(Long.parseLong(scannedCode.substring(0,i-1)));
        //eats the barcode
   ...
   * @return If the code is Smartcode - return parsing of that code
   * If the code is barcode - return Smarcode with the scanned barcode and null as expiration date
